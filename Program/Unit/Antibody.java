...
public class Antibody extends Thread implements Unit{
    int hp, atk,gain;
    int posx, posy;
    NodeTree nt;
    ...
    synchronized public void attack(String Direction) {
        ...
        int[] answer=minDistanc(Direction,1);
                ...
                if(map[answer[1]][answer[0]]!=null)
                    if(map[answer[1]][answer[0]] instanceof Virus)
                    {
                        map[answer[1]][answer[0]].getattack(this);
                        this.hp+=gain();
                        if(map[answer[1]][answer[0]].hp()<=0){
                            map[answer[1]][answer[0]]=null; VirusFactory.liveVirus--;
                            ImmuneSystem.ImmuneHandle.increasedit();
                        }
                    }
    ...
    public void run() {
        while(hp>0)
        try {
            System.out.println("ant"+posx()+" "+posy);
